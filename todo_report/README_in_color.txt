==== TODOs, ascending, so you know what's likely to change next. Do not edit, run 'generate_todo_report' to generate. ====
( showing last 8 lines out of 562 for 'todo': )
./README.md:126:  - [1m[31mTODO^125[0m[0m: move 'vital' data (to do list) to private repo (skiggety utils should help set up)
./installers/zerothlife:6:# [1m[31mTODO^140[0m[0m: (IN_PROGRESS, NOW) TEST this script
./installers/zerothlife:9:# - [1m[31mTODO^140[0m[0m: (IN_PROGRESS) make this, and maybe all installers, optional.  We could use marker files to keep track
./installers/signal:37:  # [1m[31mTODO^141[0m[0m: (IN_PROGRESS) IMPLEMENT: ...
./bin/today_todo:84:    # [1m[31mTODO^191[0m[0m: (IN_PROGRESS, NOW) also most of these paragraphs could be function extracted...:
./installers/vital:8:# [1m[31mTODO^191[0m[0m: (TESTING IN_PROGRESS, NOW) this call to that other script thingy:
./bin/today_todo:127:        # [1m[31mTODO^192[0m[0m: (TESTING IN_PROGRESS NOW) test this bit on tinhat:
[1m[33m./bin/vital:68:# [1m[31mTODO^192[0m[1m[33m: (URGENT, IN_PROGRESS, NOW) vital must help you set up, connect, and use a private repo for vital data across multiple machines, then[0m
