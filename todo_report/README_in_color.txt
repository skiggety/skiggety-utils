==== TODOs, ascending, so you know what's likely to change next.  Do not edit, run 'generate_todo_report' to generate. ====
---------------------------------------------------------------------------------------------------------------------------- todo (shown by 'abbreviate') -
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ showing last 8 lines out of 592: ^
./bin/vital:16:# [1m[31mTODO^190[0m[0m: vital should work offline
./installers/basic_prerequisites:72:    # [1m[31mTODO^192[0m[0m IN_PROGRESS NOW: install latest bash and make sure other scripts are compatible...
./bin/vimfirst-until:3:# [1m[31mTODO^200[0m[0m: this should be a special mode of vimfirst, so it shares code, just another way of using a --while flag
./bin/vimfirst-while:3:# [1m[31mTODO^200[0m[0m: this should be a special mode of vimfirst, so it shares code, just another way of using a --until flag
./bin/sleep-verbose:8:# [1m[31mTODO^212[0m[0m IN_PROGRESS: synchronize this to the clock...the logic takes time too, so over time, this script ends up taking 3% or 4% too long...
./bin/sleep-verbose:40:        sleep 1 || exit_with_error "$(basename $0) stopped" # [1m[31mTODO^226[0m[0m IN_PROGRESS: sleep longer than a second if there is still a long time to go
./bin/sleep-verbose:30:    # [1m[31mTODO^236[0m[0m IN_PROGRESS: if $delay is a clock time, sleep until that time
[1m[33m./installers/basic_prerequisites:158:# [1m[31mTODO^271[0m[1m[33m IN_PROGRESS: upgrade to asdf 0.16.0 or later ( open 'https://asdf-vm.com/guide/upgrading-to-v0-16#breaking-changes' )[0m
