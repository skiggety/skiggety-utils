...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ showing last 8 lines out of 588: ^
...
./bin/git-ready:39:    # EASY TODO^332: Just exit 0 right here unless there are changes staged (yes, we're checking again because the user
./bin/git-ready:41:    git-no-uncommitted-or-unpushed-changes && echo "No changes in $(pwd), $(basename $0) exiting early." && exit 0 # TODO^332 IN_PROGRESS TESTING NOW
./bin/git-ready:62:    run-or-fail git dashboard # TODO^332 IN_PROGRESS TESTING NOW trying to use this for its exit value
./bin/git-ready:76:exit 0 # TESTING IN_PROGRESS NOW TODO^335: only exit 0 if there is nothing to commit
