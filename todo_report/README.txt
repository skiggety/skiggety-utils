==== TODOs, ascending, so you know what's likely to change next.  Do not edit, run 'generate_todo_report' to generate. ====
---------------------------------------------------------------------------------------------------------------------------- todo (shown by 'abbreviate') -
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ showing last 8 lines out of 604: ^
./PWD_BIN/todo-report:59:limit-todo-with-keyword-count 3 'N''OW' # TODO^214 IN_PROGRESS: fine tune this number
./bin/sleep-verbose:40:        sleep 1 || exit_with_error "$(basename $0) stopped" # TODO^226 IN_PROGRESS: sleep longer than a second if there is still a long time to go
./bin/sleep-verbose:30:    # TODO^236 IN_PROGRESS: if $delay is a clock time, sleep until that time
./bin/sleep-verbose:8:# TODO^262 IN_PROGRESS: synchronize this to the clock...the logic takes time too, so over time, this script ends up taking 3% or 4% too long...
./installers/basic_prerequisites:157:# TODO^276 IN_PROGRESS: upgrade to asdf 0.16.0 or later ( open 'https://asdf-vm.com/guide/upgrading-to-v0-16#breaking-changes' )
./installers/astronvim:31:    # TODO^337 IN_PROGRESS NOW: TEST/IMPLEMENT...
./installers/github_copilot:20:    # TODO^345: (FUN IN_PROGRESS NOW) set up github copilot for nvim using skiggety-utils
./installers/github_copilot:21:    # TODO^396: (FUN IN_PROGRESS) set up github copilot for Visual Studio Code using skiggety-utils
