...
trap 'if [ "login" == "$(ps -o comm= $PPID)" ];then echo;zl-battery-status;echo;echo "================================================================================";for i in $(seq 12 1);do echo -n "~";for j in $(seq $i);do echo -n " ~ ~";done;echo;done;echo "~";echo;fortune | sed "s/^/~       /";echo;echo;echo "(Holding this shell open for a moment just in case you need to capture anything)";echo;sleep-verbose 30;oop;fi' EXIT
